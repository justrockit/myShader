Shader "Unlit/HlslUnlit"
{
    ...
    SubShader
    {
        ...
        HLSLINCLUDE
            ...
            /*
             中心点（0,0）,uv 最大 （0.5，0.5），r 最大根号2
              */
            float circular(float2 uv, float r)
            {
                return distance(uv, float2(0.5,0.5)) - r;
          ...
          中心点（0,0）,uv 最大 （0.5，0.5），r 最大根号2
           */
            float rectangle(float2 uv, float2 leftdown, float2 righttop)
            {
            ...
            HLSLPROGRAM
                ...
                half4 frag(Varyings input) : SV_Target
                {
                    //return SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, input.uv) * _BaseColor;
                       ...
                       float circuldis = abs(circular(input.uv,0.5));
                    return half4(circuldis, circuldis, circuldis,1);
                }
